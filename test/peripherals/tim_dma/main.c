/**
 * @file main.c
 * @brief This test provides a description of how to use DMA with TIM1 Update request
          to transfer Data from memory to TIM1 Capture Compare Register1.
 * @author Liyouchuang (lycdemeng@163.com)
 * @version 1.0
 * @date 2022-04-17
 * @copyright Copyright (c) 2022 Icore, Inc
 *
 * Test function    : This test provides a description of how to use DMA with TIM1 Update request
                        to transfer Data from memory to TIM1 Capture Compare Register1.
...
void test_func(void)
{
    ...
    /* GPIOA Configuration: Channel 1\1N as alternate function push-pull */
  ...
  The objective is to configure TIM1 channel 1 to generate complementary PWM
  ...
  each 3 Update Requests, the TIM1 Channel 1 Duty Cycle changes to the next new 
  ...
  The waveform can be output by oscilloscope. */
    ...
    if (NVIC_InitStructure.NVIC_IRQChannel != DMA_IRQn || TIM1->TIM_CR1.TIM_CR1_CEN != 1 ||
        ...
        TEST_ASSERT_MESSAGE(0, "Tim dma test fail");

    } else
        debug("Tim dma test success!!!\r\n");
}
